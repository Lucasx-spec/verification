...
import { signApi, verifyApi } from '../api'
...
const metrics = computed(() => [
  { label: '我的签名数', value: signRecords.value.length, accent: 'azure' },
  { label: '我的验签链接', value: verifyLinks.value.length, accent: 'orange' },
  { label: '我的验签记录', value: verifyRecords.value.length, accent: 'green' },
])

onMounted(async () => {
  loading.value = true
  try {
    const [signRes, linkRes, recordRes] = await Promise.all([
      signApi.list(),
      verifyApi.listLinks(),
  ...
  <div class="page-stack">
    <section class="hero-panel glass-card">
      <div>
        ...
        <p>这里是你的业务工作区，专注于数字签名、验签链接投放和结果回查。</p>
      </div>
      <div class="hero-actions">
        <el-button type="primary" @click="router.push('/workspace/sign')">立即签名</el-button>
        <el-button plain @click="router.push('/workspace/verify-links')">管理验签链接</el-button>
      </div>
    </section>
    ...
    <section class="dashboard-grid">
      <el-card shadow="never" class="glass-card panel-card">
        <template #header>
          <div class="panel-header">
            <span>最近签名</span>
            <el-button link @click="router.push('/workspace/sign')">查看全部</el-button>
          </div>
        </template>
      ...
      <el-card shadow="never" class="glass-card panel-card">
        <template #header>
          <div class="panel-header">
            <span>最近验签</span>
            <el-button link @click="router.push('/workspace/verify-records')">查看全部</el-button>
          </div>
        </template>
        <el-skeleton :loading="loading" animated :rows="4">
